...
exports.parse = function(file, encoding) {
  ...
  var xu = {
    ...
    */
    childOrAttr: function(node, name) {
      return this.text(node.child(name)) || this.attr(node, name);
    },
    /*
    xml转换成json
    */
  ...
  xml.child('npm').each(function(item){
    ...
    var global = beeutil.toBoolean(xu.attr(item, 'globel') || xu.attr(item, 'g'));
    name = name.split(',');
    name.forEach(function(item, index){
      if(global){
        project.npmsGlobal.push(item.trim());
      }else{
        project.npms.push(item.trim());
      }
    });
  });
  ...
  xml.child('taskdef').each(function(item) {
    ...
    var file = xu.attr(item, 'file');
    if(npmname){
      module = npmname.split(',');
      project.npms = project.npms.concat(module);
    }else if(file){
