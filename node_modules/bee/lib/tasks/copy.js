...
module.exports = function(bee) {
  ...
  bee.register('copy', function(options) {
    ...
    var file = options.src || options.file || options.dir;
    var tofile = options.tofile || options.dest || options.destfile;
    var todir = options.todir || options.dest;

    if (file) {
      if(!tofile && !todir){
        return bee.error("the dest/destfile/todir is required in copy task.");
      }
      var newname;
      if(tofile){
      ...
      if (!todir) {
        return bee.error("the dest/todir are required in copy task.");
      }
